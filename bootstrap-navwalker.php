...
class Bootstrap_NavWalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ($depth == 0) {
			...
			if ($item->description == 'linebreak') {
				$item_output = '<hr class="nav-line-break"/><div class="nav-line-break-desc">' . str_replace("%NL%", "</br>", $item->title) . '</div>';
			} else {
				$item_output .= $args->link_before;
				if (!empty( $item->attr_title)) {
					$item_output .= inline_svg($item->attr_title);
				} else {
					$item_output .= inline_svg('rose');
				}
				$item_output .= $title;
				if (!empty($item->description)) {
					$item_output .= '<br /><div class="nav-item-desc">' . str_replace("%NL%", "</br>", $item->description) . '</div>';
				}
...
function inline_svg($name) {
	//$backup = 'http://dsaredesign.local/wp-content/themes/dsausa.org-wp-v2-redesign/img/rbdsalogo.png';
	$file = get_template_directory_uri();
	$file .= "/icons/" . $name . ".svg";
	return '<img class="navbar-dsa-icon" src="' . $file . '" alt="' . $name . '" />';
}

function svg_url($name) {
	$file = get_template_directory_uri();
	$file .= "/icons/" . $name . ".svg";
	return $file;
}
